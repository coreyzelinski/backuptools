...
DEFAULT_DIRS="/var/www /etc /var/log"
...
CONFIG_FILE=${CONFIG_FILE:-$CONFIG_DIR/backup.conf}
...
#
# Set defaults. Any of these could optionally be defined in the external config file (../etc/backup.conf).
#
TAR=${TAR:-/bin/tar}
MYSQL_BACKUP=${MYSQL_BACKUP:-$THIS_DIR/mysql-backup.sh}
ROTATE_CMD=${ROTATE_CMD:-$THIS_DIR/backup-rotate.sh}
